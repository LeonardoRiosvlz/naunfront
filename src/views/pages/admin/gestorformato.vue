<template>
  <Layout>
    <PageHeader :title="title" :items="items" />
        <b-button class="mb-3" v-b-toggle.sidebar-right>Filtrar formatos</b-button>
    ...
    <div class="clearfix mb-3">
      <b-button class="float-right btn-info" left @click="$bvModal.show('modal');editMode=false;ver=false;resete();">Crear formato</b-button>
    </div>
               ...
               <template v-slot:cell(fecha_revision)="data">
                ...
                <b-dropdown size="sm" class="">
                  <template v-slot:button-content>
                    ...
                    <b-dropdown-item-button><a :href="'/gestion-versiones-form/'+data.item.id" style="color:#000;">Gestion de versiones</a></b-dropdown-item-button>
                    <b-dropdown-item-button v-if="data.item.creado==='Creado'"><a :href="'formatos/'+data.item.id" style="color:#000"> Vista al Documento </a></b-dropdown-item-button>
    ...
    <b-modal id="modal-historial" size="xl" false   title="Gestion de formatos" hide-footer>
...
export default {
  ...
  methods: {
    ...
    await this.axios.post('api/formatos/filtro',data, {
        headers: {
    ...
    await this.axios.put('api/formatos', data, {
      headers: {
       'Content-Type': 'multipart/form-data'
        ...
        this.$swal('No se pudo realizar!','','warning');
     });
    },
            ...
            if (response.status==200) {
               this.$swal(
                   'Agregado exito!',
                    '',
                    'success');
               this.$router.push({name:'gestion-versiones-form',params:{id:response.data.id}});
               this.listardocscreados();
               this.$root.$emit("bv::hide::modal", "modal", "#btnShow");
    ...
    async cargarVersions(){
     ...
     data.append('documento_id',this.form.id);
      ...
      if (this.diagrama) {
       ...
       await this.axios.post('api/formatos/version', data, {
           headers: {
            'Content-Type': 'multipart/form-data'
           }}).then(response => {
            if (response.status==200) {
